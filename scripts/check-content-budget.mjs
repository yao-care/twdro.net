 ...
 * 【2026-07-30 當時的量測，是紀錄不是現況】站上 28 篇 learn/ 教育文整週合計只賺 19 次曝光、
 * 1 次點擊，而 events/ 與 equipment/
 ...
 *
 * 還有第二層代價：learn/ 吃掉 sitemap 相當比例的網址（現況跑
 * `curl -s https://twdro.net/sitemap-0.xml | grep -o '<loc>[^<]*' | sed 's|<loc>https://twdro.net/||' \
 *   | awk -F/ '{print ($1==""?"(root)":$1)}' | sort | uniq -c | sort -rn`），而當時 10 天大、零外部
