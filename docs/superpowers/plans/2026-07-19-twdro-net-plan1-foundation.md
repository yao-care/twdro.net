...
- 部署：GitHub Pages，`astro.config.mjs` 的 `site` = `https://twdro.net`，`base` = `/`；repo 根目錄 `public/CNAME` 內容為 `twdro.net`。
...
- [ ] **Step 4：建立 `public/CNAME` 與 `public/.nojekyll`**
...
**Interfaces:**
- Produces: `BaseLayout`，props `{ title: string; description?: string; lang?: string }`，import `tokens.css` 一次，提供 `<slot />`。所有頁面套此 layout。
...
```bash
git add -A
git commit -m "feat: design-tokens 全域 CSS 與 BaseLayout
...
<header style="border-bottom:1px solid var(--border-subtle);background:var(--bg-surface)">
  <div class="container" style="display:flex;gap:1.5rem;align-items:center;padding-top:1rem;padding-bottom:1rem;flex-wrap:wrap">
    <a href="/" style="font-weight:700;font-size:var(--text-lg);text-decoration:none">twdro.net</a>
    <nav style="display:flex;gap:1.2rem;flex-wrap:wrap">
      {nav.map(([href, label]) => <a href={href} class="fs-sm">{label}</a>)}
    </nav>
  </div>
</header>
```
  ...
  <ul>
    {articles.map((a) => (
      <li style="margin:.6rem 0">
        <a href={`/learn/${a.id}`} class="fs-lg">{a.data.title}</a>
...
<BaseLayout title="規則總覽 | twdro.net">
  ...
  {systems.map((sys) => {
    ...
    return list.length ? (
      <section>
        <h2>{sys}</h2>
        <ul>{list.map((b) => <li><a href={`/rules/${b.id}`} class="fs-lg">{b.data.name}</a></li>)}</ul>
      </section>
    ) : null;
  })}
</BaseLayout>
  ...
  <ul style="list-style:none;padding:0">
    {events.map((e) => (
      <li style="border:1px solid var(--border-subtle);border-radius:.5em;padding:1rem;margin:.8rem 0;background:var(--bg-surface)">
        ...
        <a href={`/events/${e.id}`} class="fs-lg" style="display:block;margin-top:.4rem">{e.data.title}</a>
...
<BaseLayout title="台灣無人機足球的一站式入口 | twdro.net">
  <section style="text-align:center;padding:2rem 0">
    ...
    <p style="margin-top:1.5rem">
      <a href="/events" class="fs-lg" style="margin:0 .8rem">查看近期賽事</a>
      <a href="/learn" class="fs-lg" style="margin:0 .8rem">認識無人機足球</a>
    </p>
  </section>
  ...
  {next && (
    <section style="border:1px solid var(--border-subtle);border-radius:.5em;padding:1.5rem;background:var(--bg-surface)">
      ...
      <a href={`/events/${next.id}`} class="fs-xl" style="display:block;margin:.5rem 0">{next.data.title}</a>
    ...
    <ul style="list-style:none;padding:0">
      {recent.map((e) => (
        <li style="padding:.6rem 0;border-bottom:1px solid var(--border-subtle)">
          <StatusBadge status={e.data.status} />
          <a href={`/events/${e.id}`} style="margin-left:.5rem">{e.data.title}</a>
        </li>
      ))}
    </ul>
  </section>
...
<BaseLayout title="平台使命 | twdro.net">
  ...
  <p class="fs-sm text-muted">相關頁面：
    <a href="/about/sources">資料來源</a>、
    <a href="/about/correction">回報資料錯誤</a>、
    <a href="/about/privacy">隱私權政策</a>、
    <a href="/about/terms">使用條款</a>、
    <a href="/about/disclaimer">免責聲明</a>
  </p>
</BaseLayout>
```
...
<BaseLayout title="隱私權政策 | twdro.net">
  ...
  <ul>
    ...
    <li>提供監護人申請移除機制（見<a href="/about/correction">回報頁</a>）。</li>
  </ul>
</BaseLayout>
```
...
## 部署
push 到 `main` 由 `.github/workflows/deploy.yml` 自動建置並發佈至 GitHub Pages（`twdro.net`）。
```
