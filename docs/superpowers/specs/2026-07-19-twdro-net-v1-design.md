...
- 本文件範圍：**v1（靜態內容層）**。v2/v3 僅列為長期藍圖，不在本次實作。
...
```
v1 靜態內容層（GitHub Pages）        ← 本文件
   賽事入口 + 規則百科 + 資料庫 + 合規檢查器 + 規則比較器 + 搜尋 + SEO
        ↓
v2 互動層（Cloudflare Workers/D1）
   會員、隊伍認領、訂閱通知、AI 規則助理、社群媒合、選手個人頁
        ↓
v3 賽事管理 SaaS
   線上報名、賽程生成、即時比分、電子檢錄、主辦單位後台
```

每個里程碑各自一份 spec，做完再往下。
...
- **v1 完全不碰選手個資。** public repo 僅含賽事/規則/隊伍/場地/器材/組織。
- 得獎名單只到**隊伍層級**。
- 無選手個人頁（整塊留 v2 有帳號 + 權限時）。
...
- 全國積分排名。
- 選手公開個人頁、會員系統、線上報名、即時比分、伺服器端 AI 助理、原生 App、社群討論區。
